...
追加
済　textから曲名とスピードなどの設定を取り込めるようにする(曲名、スピード、ボリューム、キー設定など)
済　miss判定の追加
済　リザルト画面の作成
（スコア、各判定の合計、最大コンボ数、フルコンボか否か）
...
#ゲームプレイ用の設定１ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
window_bg ="#222222"
...
#ゲームプレイ用の設定２(ゲームの設定読み込み)　　　　　ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー
setteilist = []
def read_S():
    settei = open("設定.txt")
    for date in settei :
        itiji = date.strip("\n")
        itiji = itiji.split("/")
        setteilist.append(itiji[1])   #settteilistのの中に一項目ずつ入れていく
    global kyoku , fail , M_volume ,key1 ,key2 , key3 ,key4
    kyoku     = setteilist[0]   #曲名の指定
    fail      = setteilist[1]
    M_volume  = int(setteilist[2])
    key1      = setteilist[3]
    key2      = setteilist[4]
    key3      = setteilist[5]
    key4      = setteilist[6]
...
#リザルト画面の表示
def result ():
    resultlabel = tk.Label(window , font = FONT ,width=600 ,height=500)
    resultlabel.pack()
    if combo_max == perfect_n+great_n+good_n+miss_n :
        resultlabel["text"] ="～リザルト～" +"\n\n"\
                            +"   スコア   " + str(score)        +"\n" \
                            +"最大コンボ数　" + str(combo_max)   +"\n" \
                            +"   perfect :" + str(perfect_n)    +"\n" \
                            +"    great  :" + str(great_n)      +"\n" \
                            +"    good   :" + str(good_n)       +"\n" \
                            +"    miss   :" + str(miss_n)       +"\n" \
                            +"\n" +"フルコンボ！"
...
def readG ():
    ...
    global G_gosa ,speed
    speed = int(G_list.pop(0))     #一列目は落下速度の設定
...
def scorepuls (hyouka):
    global score ,perfect_n ,great_n ,good_n ,miss_n
    if hyouka == "perfect" :
        score += 100
        perfect_n += 1
        combopuls()
